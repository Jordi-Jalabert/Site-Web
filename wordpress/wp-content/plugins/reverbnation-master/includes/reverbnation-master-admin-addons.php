<?php
function menu_single_reverbnation_admin_addons(){
	if ( is_admin() )
	add_submenu_page( 'reverbnation-master', 'Add-ons', 'Add-ons', 'manage_options', 'reverbnation-master-admin-addons', 'reverbnation_master_admin_addons' );
}

function reverbnation_master_admin_addons(){
$plugin_master_name = constant('REVERBNATION_MASTER_NAME');
?>
<div class="wrap">
<h1><?php echo $plugin_master_name; ?> Add-ons</h1>
<?php
if(!class_exists('reverbnation_master_admin_addons_table')){
	require_once( WP_PLUGIN_DIR . '/reverbnation-master/includes/reverbnation-master-admin-addons-table.php');
}
//Prepare Table of elements
$wp_list_table = new reverbnation_master_admin_addons_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'admin_menu', 'menu_single_reverbnation_admin_addons' );
}
else {
add_action( 'admin_menu', 'menu_single_reverbnation_admin_addons' );
}
